...
2. **OXI One accuracy.** Preserve correct facts already in content: Sequencer 7 for the kit, Sequencer 8 for the second instrument (only Latin/Clave uses Seq 8 today). Timing offsets are expressed as % of a 16th-note step (−45% to +45% range). Keep build instructions consistent with the actual pattern.
